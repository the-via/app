...
function generateKeycodeDict(
  ...
  const keycodesObjs2: Record<number, Object & {key: string}> = Object.entries(
    ...
    const key = Number.parseInt(byte);
    if (!Number.isNaN(key)) {
      return {...p, [key]: obj};
    } else {
      return p;
  ...
  const keycodes: Record<string, Object> = Object.entries(keycodesObjs3).reduce(
    (p, [key, obj]) => {
      if (key.startsWith('QK_KB_') || key.startsWith('QK_MACRO_')) {
        return p;
      } else {
        return {...p, [key]: obj};
      }
